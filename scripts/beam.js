...
class Beam {
  ...
  constructor( init_obj ){
    ...
    // Config
    this.color = '#ffffdc';
    this.range_x = [ (canvas.width / 2) - 150, (canvas.width / 2) + 150];
    this.range_y = [ (canvas.height / 2), -100];
    ...
    this.height = (Math.random() * (25 - 55)) + 70;
    
    // Start X & Y
    this.start_x = (Math.random() * (this.range_x[0] - this.range_x[1])) + this.range_x[1];
    this.start_y = (Math.random() * ((canvas.height / 2) - 5) - (canvas.height/2)) + (canvas.height / 1);
